...
/** Text-safe spectrum cut — clears 4.5:1 at every value. */
export const signalInk = (v: number) => `oklch(${(0.3 + 0.15 * v).toFixed(3)} 0.13 ${hue(v).toFixed(1)})`;
...
export function RankBadge({
  ...
  return (
    <span
      className={`inline-flex h-5 min-w-7 items-center justify-center rounded px-1 font-mono text-xs font-semibold tabular-nums ${className}`}
      style={{
